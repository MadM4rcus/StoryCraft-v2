            ...
            {block.type === 'text' ? (
                block.isCollapsed ? (
                    <div className="cursor-pointer" onClick={() => updateNoteBlock(block.id, 'isCollapsed', false)}>
                        <p className="text-lg font-semibold mb-1 text-textPrimary">Anotação de Texto</p>
                        <p className="text-sm italic text-textSecondary truncate">{block.value || 'Vazio...'}</p>
                    </div>
                ) : (
                    <>
                        <AutoResizingTextarea value={block.value} onChange={(e) => updateNoteBlock(block.id, 'value', e.target.value)} placeholder="Digite sua anotação..." className="w-full p-2 bg-bgInput border border-bgElement rounded-md text-textPrimary" />
                        <div onClick={() => updateNoteBlock(block.id, 'isCollapsed', true)} className="mt-2 text-center p-1 bg-bgInput hover:opacity-80 text-xs font-bold rounded-md cursor-pointer text-textSecondary">
                            Ocultar ▲
                        </div>
                    </>
                )
                ...
                block.isCollapsed ? (
                    ...
                    <div className="flex flex-col items-center">
                        ...
                        <div className="flex flex-wrap items-center justify-center gap-4 mt-2 text-sm text-textSecondary">
                            <label><input type="checkbox" checked={block.fitWidth} onChange={(e) => updateNoteBlock(block.id, 'fitWidth', e.target.checked)} className="form-checkbox text-btnHighlightBg rounded" /> Ajustar à Largura</label>
                            {!block.fitWidth && (
                                <>
                                    <label>Largura (px): <input type="number" value={block.width === 0 ? '' : block.width} onChange={(e) => updateNoteBlock(block.id, 'width', parseInt(e.target.value) || 0)} className="w-20 p-1 bg-bgInput border border-bgElement rounded-md text-center text-textPrimary" /></label>
                                    <label>Altura (px): <input type="number" value={block.height === 0 ? '' : block.height} onChange={(e) => updateNoteBlock(block.id, 'height', parseInt(e.target.value) || 0)} className="w-20 p-1 bg-bgInput border border-bgElement rounded-md text-center text-textPrimary" /></label>
                                </>
                            )}
                        </div>
                        <div onClick={() => updateNoteBlock(block.id, 'isCollapsed', true)} className="mt-2 w-full text-center p-1 bg-bgInput hover:opacity-80 text-xs font-bold rounded-md cursor-pointer text-textSecondary">
                            Ocultar ▲
                        </div>
                    </div>
                )
            ...
            {!isCollapsed && (
                <>
                    <div className="space-y-4 mb-4">
                        {(character.notes || []).length === 0 ? <p className="text-textSecondary italic">Nenhum bloco de anotação adicionado.</p> : character.notes.map((block) => renderBlock(block))}
                    </div>
