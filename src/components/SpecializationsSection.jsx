// src/components/SpecializationsSection.jsx
...
const SpecializationsSection = ({ character, isMaster, onUpdate, isCollapsed, toggleSection }) => {
    ...
    const toggleItemCollapsed = (id) => {
        ...
        onUpdate('specializations', newList);
    };
                        ...
                        {(character.specializations || []).map(spec => (
                            <div key={spec.id} className="flex flex-col p-3 bg-bgElement rounded-md shadow-sm">
                                <div className="flex justify-between items-center mb-1">
                                    <span className="font-semibold text-lg w-full cursor-pointer text-textPrimary" onClick={() => toggleItemCollapsed(spec.id)}>
