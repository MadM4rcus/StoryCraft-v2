...
const AttributesSection = ({ character, isMaster, onUpdate, isCollapsed, toggleSection }) => {
              ...
              return (
                <div key={attr.id} className="p-3 bg-gray-600 rounded-md shadow-sm border border-gray-500 relative flex flex-col gap-3">
                  <div className="flex items-center gap-2 cursor-pointer" onClick={() => handleToggleCollapsed(attr.id)}>
                    <input type="text" placeholder="Nome do Atributo" value={attr.name} onClick={(e) => e.stopPropagation()} onChange={(e) => handleAttributeChange(attr.id, 'name', e.target.value)} className="w-full flex-grow p-2 bg-gray-700 border border-gray-500 rounded-md text-white font-semibold cursor-text" disabled={!isMaster} />
                  ...
                  <div className="flex items-center gap-1 sm:gap-2 text-xs justify-end w-full" onClick={(e) => e.stopPropagation()}>
                    {['base', 'perm', 'arma'].map(field => (
                      <div key={field} className="flex flex-col items-center">
                        ...
                        <input type="number" value={attr[field] || ''} onChange={(e) => handleAttributeChange(attr.id, field, e.target.value)} className={`w-12 p-1 border rounded-md text-white text-center bg-gray-700 border-gray-500`} disabled={!isMaster} />
                      </div>
                    ))}
                  ...
                  {isMaster && (
                    <div className="flex justify-start pt-2 mt-2 border-t border-gray-500/50" onClick={(e) => e.stopPropagation()}>
                      <button onClick={() => handleRemoveAttribute(attr.id)} className="px-3 py-1 bg-red-600 hover:bg-red-700 text-white text-xs font-bold rounded-md">
                        Remover
                      </button>
                    </div>
                  )}
